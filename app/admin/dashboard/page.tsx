...
const menuItems = [
  ...
  { name: "Custom Ticket", href: "/admin/custom-booking", icon: BookUser },
...
// Accent rotation for stat cards so the row isn't monochrome
const STAT_ACCENTS = [
  { bg: "bg-teal-50", ring: "ring-teal-100", icon: "text-teal-700" },
  { bg: "bg-indigo-50", ring: "ring-indigo-100", icon: "text-indigo-600" },
  { bg: "bg-amber-50", ring: "ring-amber-100", icon: "text-amber-600" },
  { bg: "bg-rose-50", ring: "ring-rose-100", icon: "text-rose-600" },
];
...
export default function AdminDashboard() {
  ...
  const [tripCategory, setTripCategory] = useState<TripStatus>("today");
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, bookingsRes, tripsRes] = await Promise.all([
        ...
        fetch("/api/trips/departures", { cache: "no-store" }),
      ]);

      if (!statsRes.ok) throw new Error("Failed to fetch stats");
      if (!bookingsRes.ok) throw new Error("Failed to fetch bookings");
      if (!tripsRes.ok) throw new Error("Failed to fetch trip departures");
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      const fetched: AdminNotification[] = data.notifications || [];
      setNotifications(fetched);

      const hasReadFlag = fetched.some((n) => typeof n.read === "boolean");
      setNotificationCount(
        hasReadFlag ? fetched.filter((n) => !n.read).length : data.count || 0
      );
    } catch (error) {
  ...
  async function handleDeleteTrip() {
    if (!tripToDelete?.busId || deletingTripId) return;

    const tripId = tripToDelete.busId;
    setDeletingTripId(tripId);

    try {
      const res = await fetch(`/api/admin/busses/${tripId}`, {
        method: "DELETE",
      });
      ...
      setPreviousTrips((prev) => prev.filter((trip) => trip.busId !== tripId));
      setTodayTrips((prev) => prev.filter((trip) => trip.busId !== tripId));
      setUpcomingTrips((prev) => prev.filter((trip) => trip.busId !== tripId));
      setBookings((prev) =>
        prev.filter((booking) => booking.bus?._id !== tripId)
      );
  ...
  const statCards = [
    { title: "Total Bookings", value: stats?.bookedSeats ?? "—", icon: CalendarCheck },
    { title: "Today's Bookings", value: stats?.todaysBookedSeats ?? "—", icon: TrendingUp },
    { title: "Active Buses", value: stats?.totalBuses ?? "—", icon: Bus },
    { title: "Passengers", value: stats?.bookedSeats ?? "—", icon: Users },
  ];
    ...
    <div className="min-h-screen bg-slate-50">
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon
                    size={19}
                    className={isActive ? "text-teal-300" : "text-white/40 group-hover:text-teal-300"}
                  />
                  <span>{item.name}</span>
          ...
          <nav className="space-y-1">
            {bottomItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon size={19} className="text-white/40 group-hover:text-teal-300" />
                  {item.name}
                </Link>
              );
            })}
            ...
            >
              <LogOut size={19} className="text-white/40 group-hover:text-rose-300" />
              Log Out
            </button>
          </nav>
        </div>
        ...
        <div className="border-t border-white/10 p-4">
          <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-xs text-white/40">admin@haikaltours.com</p>
            </div>
            <button
              type="button"
              onClick={() => setConfirmLogout(true)}
      ...
      <main className="lg:ml-[270px]">
        ...
        <header className="sticky top-0 z-30 flex h-[76px] items-center justify-between border-b border-slate-100 bg-white/90 px-4 backdrop-blur-md sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-900 sm:text-xl">Dashboard</h1>
              ...
              return (
                ...
                >
                  <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${accent.bg} ring-1 ${accent.ring}`}>
                    <Icon size={21} className={accent.icon} />
                  </div>
                  ...
                  >
                    <div>
                      <p className="text-sm font-bold text-slate-900">
                        {b.passengerName}
                        <span className="font-normal text-slate-400"> · {b.bookingRef}</span>
                      </p>
            ...
            <div className="rounded-2xl border border-slate-100 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 p-5">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">Latest customer reservations</p>
                </div>
                      ...
                      >
                        <td className="px-5 py-4">
                          <Link href={`/admin/bookings/${booking._id}`} className="block">
                            <p className="text-xs font-bold text-slate-800">{booking.bookingRef}</p>
                ...
                {paginatedBookings.map((booking) => (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-xs font-bold text-slate-400">{booking.bookingRef}</p>
                        <p className="mt-1 font-bold text-slate-900">{booking.passengerName}</p>
                      </div>
              ...
              {bookings.length > 0 && (
                <div className="flex flex-col items-center justify-between gap-3 border-t border-slate-100 px-5 py-4 sm:flex-row">
                  <p className="text-xs text-slate-400">
                    ...
                    of <span className="font-bold text-slate-600">{bookings.length}</span> bookings
                  </p>
              ...
              <div className="border-b border-slate-100 p-4 sm:p-5">
                <div className="flex flex-col gap-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h3 className="font-bold text-slate-900">Trip Departures</h3>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 rounded-xl bg-slate-100 p-1">
                    <button
                      type="button"
                      onClick={() => setTripCategory("previous")}
                    ...
                    >
                      Previous
                      <span className="ml-1 opacity-60">({tripCounts.previous})</span>
                    </button>

                    <button
                      type="button"
                      onClick={() => setTripCategory("today")}
                      className={`rounded-lg px-2 py-2 text-[10px] font-black transition sm:text-xs ${
                        tripCategory === "today"
                    ...
                    >
                      Today
                      <span className="ml-1 opacity-60">({tripCounts.today})</span>
                    </button>

                    <button
                      type="button"
                      onClick={() => setTripCategory("upcoming")}
                      className={`rounded-lg px-2 py-2 text-[10px] font-black transition sm:text-xs ${
                        tripCategory === "upcoming"
                          ? "bg-white text-blue-700 shadow-sm"
                          : "text-slate-500 hover:text-slate-700"
                      }`}
                    >
                      Upcoming
                      <span className="ml-1 opacity-60">({tripCounts.upcoming})</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="divide-y divide-slate-100">
                ...
                {!loading && activeTrips.length === 0 && (
                  <div className="px-5 py-10 text-center">
                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100">
                      <Bus size={21} className="text-slate-400" />
                    </div>
                ...
                {!loading &&
                  activeTrips.map((trip) => {
                    ...
                    const booked = Number(
                      trip.bookedSeats ?? trip.passengerCount ?? 0
                    );
                    const occupancy =
                      capacity > 0
                        ? Math.min((booked / capacity) * 100, 100)
                        : 0;
                    ...
                    return (
                      ...
                      >
                        <div className="absolute right-3 top-3 z-20 sm:right-4 sm:top-4">
                          <button
                            type="button"
                            aria-label={`Trip actions for ${trip.busName || trip.busNumber || "Bus"}`}
                            onClick={(event) => {
                              event.stopPropagation();
                              setTripMenuId((current) =>
                                current === trip.busId ? null : trip.busId
                              );
                            }}
                            className={`flex h-8 w-8 items-center justify-center rounded-lg border transition sm:h-9 sm:w-9 ${
                              tripMenuId === trip.busId
                          ...
                          <div className="min-w-0 flex-1">
                              ...
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <p className="truncate text-sm font-black text-slate-900">
                                    {trip.busName || trip.busNumber || "Bus"}
                                  </p>

                                  {trip.busNumber &&
                                    trip.busName &&
                                    trip.busNumber !== trip.busName && (
                            ...
                            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-slate-400">
                                ...
                                {trip.price != null && (
                                  <span className="font-semibold">
                                    Rs. {Number(trip.price).toLocaleString()}
                                  </span>
                                )}
                                {trip.bookingCount != null && (
                                  <span>
                                    {trip.bookingCount} booking
                                    {trip.bookingCount !== 1 ? "s" : ""}
                                  </span>
                                )}
                              </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div>
                <h3 className="text-lg font-black text-slate-900">Booking Notifications</h3>
                <p className="mt-1 text-xs text-slate-400">
            ...
            <div className="max-h-[500px] overflow-y-auto">
              {unreadNotifications.length === 0 ? (
                <div className="px-5 py-12 text-center">
                  ...
                  <p className="mt-4 text-sm font-bold text-slate-700">No new notifications</p>
                  <p className="mt-1 text-xs text-slate-400">
          ...
          >
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-100 bg-white px-3 py-3 sm:px-6 sm:py-5">
              <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-rose-50 sm:h-10 sm:w-10 sm:rounded-xl">
                  <CalendarCheck size={17} className="text-rose-500 sm:h-[19px] sm:w-[19px]" />
                </div>
              ...
              <div className="rounded-xl border border-slate-100 p-3 sm:rounded-2xl sm:p-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <span className="shrink-0 text-xs text-slate-400 sm:text-sm">Name</span>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <span className="shrink-0 text-xs text-slate-400 sm:text-sm">Phone</span>
                    <span className="min-w-0 truncate text-right text-xs font-bold text-slate-900 sm:text-sm">
                  ...
                  {selectedNotification.bookingId.passengerEmail && (
                    <div className="flex items-center justify-between gap-3">
                      <span className="shrink-0 text-xs text-slate-400 sm:text-sm">Email</span>
                      <span className="min-w-0 truncate text-right text-xs font-bold text-slate-900 sm:text-sm">
                        {selectedNotification.bookingId.passengerEmail}
                      </span>
                    </div>
              ...
              <div className="rounded-xl border border-slate-100 p-3 sm:rounded-2xl sm:p-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-2.5 sm:gap-3">
                    <MapPin size={16} className="mt-0.5 shrink-0 text-teal-700 sm:h-[17px] sm:w-[17px]" />
                    <div className="min-w-0">
                      <p className="text-[9px] text-slate-400 sm:text-[10px]">Route</p>
                  ...
                  <div className="flex items-start gap-2.5 sm:gap-3">
                    <Bus size={16} className="mt-0.5 shrink-0 text-teal-700 sm:h-[17px] sm:w-[17px]" />
                    <div className="min-w-0">
                      <p className="text-[9px] text-slate-400 sm:text-[10px]">Bus</p>
                  ...
                  <div className="flex items-start gap-2.5 sm:gap-3">
                    <Clock3 size={16} className="mt-0.5 shrink-0 text-teal-700 sm:h-[17px] sm:w-[17px]" />
                    <div className="min-w-0">
                      <p className="text-[9px] text-slate-400 sm:text-[10px]">Departure</p>
                      <p className="text-xs font-bold text-slate-900 sm:text-sm">
                        {selectedNotification.bookingId.travelDate
                          ? new Date(selectedNotification.bookingId.travelDate).toLocaleDateString()
                          : "—"}
                        {" · "}
            ...
            <div className="shrink-0 border-t border-slate-100 bg-white p-3 sm:p-4">
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <button
                  type="button"
                  disabled={actioningId === selectedNotification.bookingId._id}
                  onClick={() =>
                    handleBookingAction(selectedNotification.bookingId!._id, "reject")
                  }
                ...
                <button
                  ...
                  disabled={actioningId === selectedNotification.bookingId._id}
                  onClick={() =>
                    handleBookingAction(selectedNotification.bookingId!._id, "approve")
                  }
        ...
        >
          ...
          >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-1 text-[9px] font-black ${
                      selectedTrip.status === "previous"
                        ? "bg-slate-100 text-slate-600"
                        : selectedTrip.status === "today"
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-6">
              <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                <div className="rounded-xl bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-xs font-black text-slate-800">
                    {selectedTrip.date
                      ? new Date(`${selectedTrip.date}T00:00:00`).toLocaleDateString(
                          undefined,
                          {
                            day: "2-digit",
                            month: "short",
              ...
              {(selectedTrip.passengers?.length || 0) === 0 ? (
                ...
                <div className="space-y-3">
                  {selectedTrip.passengers?.map((passenger, index) => (
                    <div
                      key={`${passenger.id || passenger.bookingRef || "passenger"}-${index}`}
                          ...
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            <div className="flex min-w-0 items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-2">
                              ...
                              <span className="truncate text-xs font-semibold text-slate-600">
                                {passenger.phone || "Phone not available"}
                              </span>
                            </div>
                            ...
                            <div className="flex min-w-0 items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-2">
                              ...
                              <span className="truncate text-xs font-semibold text-slate-600">
                                {passenger.email || "Email not available"}
                              </span>
                            </div>
                          </div>
      ...
      {tripToDelete && (
        <div
          ...
          onClick={() => !deletingTripId && setTripToDelete(null)}
        >
          <div
          ...
          >
            <div className="p-6 sm:p-7">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-50 ring-1 ring-rose-100">
                <Trash2 size={23} className="text-rose-500" />
              </div>
              ...
              <div className="mt-5 rounded-2xl bg-slate-50 p-4">
                <p className="text-xs font-black text-slate-900">
                  {tripToDelete.busName || tripToDelete.busNumber || "Bus"}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  {tripToDelete.route ||
                    `${tripToDelete.pickup || "—"} → ${tripToDelete.dropoff || "—"}`}
                </p>
                <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-semibold text-slate-400">
                  {tripToDelete.date && <span>{tripToDelete.date}</span>}
                  {tripToDelete.departure && <span>· {tripToDelete.departure}</span>}
                  <span>· {tripToDelete.passengerCount ?? 0} passenger{(tripToDelete.passengerCount ?? 0) !== 1 ? "s" : ""}</span>
                </div>
              </div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                ...
                <button
                  ...
                  disabled={deletingTripId === tripToDelete.busId}
                ...
                >
                  {deletingTripId === tripToDelete.busId ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (
                    <Trash2 size={16} />
                  )}
                  {deletingTripId === tripToDelete.busId ? "Deleting..." : "Delete trip"}
                </button>
              </div>
            </div>
          </div>
